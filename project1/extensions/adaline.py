...
class Adaline():
    ...
    '''
    def __init__(self, n_epochs=1000, lr=0.001, lr_rate=1.00, lr_min=0.001, regularization_rate=0, loss_based_lr=False):
        '''
        Parameters:
        ----------
        n_epochs: (int)
        ...
        self.lr_min = lr_min
        self.regualarization_rate = regularization_rate

        self.loss_based_lr = loss_based_lr
    ...
    def compute_loss(self, y, net_act):
        ...
        loss = sum_squared_error*0.5 + self.regualarization_rate*np.sum(self.wts)
        return loss
    ...
    def fit(self, features, y,early_stopping=False, loss_tol=0.1):
        ...
        if (early_stopping):
            ...
            while(loss_diff>loss_tol and i < self.n_epochs):
                ...
                self.lr = max(self.lr*self.lr_rate, self.lr_min)
            ...
            for i in range(self.get_num_epochs()):
                ...
                # update lr
                if self.loss_based_lr:
                    self.lr *= min(loss, 1000)/1000
